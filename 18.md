...
- `ms`: Whether the mint list is strict. Defaults to `true` if `m` is set
- `fr`: Additional fee reserve, in the request unit, that the payer SHOULD add when paying from a mint outside a non-strict mint list
- `sm`: A set of payment methods the mint from which the ecash is sent MUST support
...
If `ms` is not present, or is `true`, the mint list is strict: the receiver MUST reject payments from mints not in the list.

If `ms` is `false`, the mint list is preferred: the receiver accepts payments from other mints, but the payer SHOULD use a mint in `m` when possible. If the payer sends ecash from a mint outside `m` and `fr` is set, the payer SHOULD add `fr` to the requested amount as a fee reserve.
