...
This brings *"silent payments"* to Cashu: Proofs can be locked to a well known public key, posted in public without compromising privacy, and spent by the recipient without needing any side-channel communication.
...
2. For each receiver key `P`, compute:
   ...
   c. `P′ = P + rᵢ·G`.
2. Build the canonical P2PK secret with the blinded `P′` keys in their slots.
3. Interact with the mint normally; the mint never learns `P` or `rᵢ`.
4. Include `p2pk_e = E` in the final proof.
