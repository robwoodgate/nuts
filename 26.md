...
For P2BK, the sender creates an ephemeral keypair (private key: `e`, public key: `E`). This protects the privacy of their own long-lived public key.

They calculate the shared secret by combining the ephemeral private key (`e`) and the receiver's long-lived public key (`P`).
...
This shared secret then used to derive the blinded public keys.

## Deriving Blinded Public Keys

Each public key in the NUT-11 proof is permanently blinded using a deterministic blinding scalar (`rᵢ`), where `i` is the _slot index_ in the order: `[data, ...pubkeys, ...refund]`.
...
Where `p` is their long lived private key.
...
3. For each slot `i`, compute: \
   ...
   b. Compute `Rᵢ = rᵢG` \
   c. Unblind `P = P' − Rᵢ` \
   c. Verify `x(P) == x(pG)`. If it does not match, this `P'` is not for this private key, skip it. \
   d. Derive the secret key using: \
